...
if __name__ == '__main__':
    ...
    parser.add_argument('-checkpoint', default='/root/VG-GPLMs/src/checkpoint/_30_multi_modal_bart_attntype4_headnums4/last.ckpt', type=str, help='The checkpoint path') # 처음 시작할때는 가져올 checkpoint 없음 -> default = 'None' . '-checkpoint', default='여기다가 ㄱㄱ', type=str,
    ...
    parser.add_argument('-val_save_file', default='/root/VG-GPLMs/src/evaluation/temp_valid_file_30_multimodal_bart', type=str, help='the validation results for each epoch') # 기존은 /temp_valid_file
    ...
    parser.add_argument('-batch_size', type=int, default=8, help='batch size for each gpu')     # 원래가 4  // 120 시도했는데 capacity 부족 뜸  // 32로 새로 돌려볼게
    ...
    parser.add_argument('-num_epochs', type=int, default=150, help='maximun number of training epoches') # 원래는100
    ...
    # set logger
    logger = pl_loggers.TensorBoardLogger(f'./lightning_logs/_30_multi_modal_bart')  # log_name = multi_modal_t5

    # save checkpoint
    checkpoint_callback = ModelCheckpoint('./checkpoint/_30_multi_modal_bart_attntype4_headnums4',
