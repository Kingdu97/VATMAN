...
if __name__ == '__main__':
    ...
    parser.add_argument('-checkpoint', default ='', type=str, help='The checkpoint path') # 처음 시작할때는 가져올 checkpoint 없음. default = '' 이거 하면 시작도 안됨. '-checkpoint', default='여기다가 ㄱㄱ', type=str,
    ...
    parser.add_argument('-val_save_file', default='/root/VG-GPLMs/src/evaluation/temp_valid_file_multi_modal_bart', type=str, help='the validation results for each epoch') # 기존은 /temp_valid_file
    ...
    parser.add_argument('-log_name', default='multi_modal_bart', type=str, help='lightning log path')   # 모델에 따라 바꿔줘야함. lightning log로 저장해줌 
    ...
    parser.add_argument('-img_lr_factor', type=float, default=5, help='the learning rate for visual guidance part will times this number')
    ...
    => T_out = T + T_out (Residual Connection)
    '''
    parser.add_argument('-cross_attn_type', type=int, default=0)  # 원래는 0이였는데 현재 멀모T5라서 __로 넣어놈 # text_only_T5 0 하겠음. # 멀모bart 4로 하겠음
    ...
    # save checkpoint
    checkpoint_callback = ModelCheckpoint('./checkpoint/multi_modal_bart',
