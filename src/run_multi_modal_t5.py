...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', default='multi_modal_t5', type=str, help='We have for models to choose, text_only_bart, multi_modal_bart,  text_only_t5 and multi_modal_t5')
    parser.add_argument('-checkpoint', default='/root/VG-GPLMs/src/checkpoint/multi_modal_t5_attntype4_headnums4/last.ckpt', type=str, help='The checkpoint path') # 처음 시작할때는 가져올 checkpoint 없음 -> default = 'None' . '-checkpoint', default='여기다가 ㄱㄱ', type=str,
    parser.add_argument('-train_src_path', default='./dataset300/sum_train_300/tran.tok.txt', type=str, help='training input data path (dialogue)') #####여기서부터 train 시에 바꿀필요 없음
    parser.add_argument('-train_tgt_path', default='./dataset300/sum_train_300/desc.tok.txt', type=str, help='training output data path (summary)') 
    parser.add_argument('-val_src_path', default='./dataset300/sum_valid_300/tran.tok.txt', type=str, help='validatioin input data path (dialogue)')
    parser.add_argument('-val_tgt_path', default='./dataset300/sum_valid_300/desc.tok.txt', type=str, help='validatioin output data path (summary)')
    parser.add_argument('-test_src_path', default='./dataset300/sum_test_300/tran.tok.txt', type=str, help='testing input data path (dialogue)')
    parser.add_argument('-test_tgt_path', default='./dataset300/sum_test_300/desc.tok.txt', type=str, help='testing output data path (summary)')
    parser.add_argument('-image_feature_path', default='./dataset300/video_features_300/', type=str, help='video features path') #####여기까지는 train 시에 바꿀필요 없음
    parser.add_argument('-val_save_file', default='./evaluation/temp_valid_file_multimodal_t5', type=str, help='the validation results for each epoch') # 기존은 /temp_valid_file
    ...
    parser.add_argument('-log_name', default='multi_modal_t5', type=str, help='lightning log path')   # 모델에 따라 바꿔줘야함. lightning log로 저장해줌 
    parser.add_argument('-gpus', default='0,1,2,3', type=str, help='choose gpus to run the code, you can choose multipple gpus') # 0,1,2,3 했었어야함 그래야 4개
    parser.add_argument('-batch_size', type=int, default=8, help='batch size for each gpu')     # 원래가 4  // 120 시도했는데 capacity 부족 뜸 
    ...
    parser.add_argument('-n_beams', type=int, default=5, help='the number of beams using for generation') # 원랜4 mulmobart 5 mulmot5 5
    ...
    parser.add_argument('-num_epochs', type=int, default=50, help='maximun number of training epoches') # 원래는100
    ...
    parser.add_argument('-do_train', type=str, default='True', help='set True to training, set False to not training') # 이거 그냥 True라고하면 안돌아감 'True'꼭
    ...
    => T_out = T + T_out (Residual Connection)
    '''
    parser.add_argument('-cross_attn_type', type=int, default=4) # only needed when cross_attn_type is 4 or 5  # 원래는 0이였는데 현재 멀모T5에도 0 해버림;; # text_only_T5 0 하겠음. # 멀모bart 4로 하겠음
    parser.add_argument('-dim_common', type=int, default=256) ## 위에 attn_type을 개같이 놨었네;;; head로만 놨었음 ;;
    parser.add_argument('-n_attn_heads', type=int, default=4) # only needed when cross_attn_type is 4 or 5  # 원래는 1이였는데 현재 멀모T5라서 4로 넣어놈 # text_only_T5 1 하겠음. # 멀모bart 4로 하겠음
                        # attention head 수를 임베딩 차원(512)의 약수가 되어야함.
    # Add to decoding
    ...
    # set logger
    logger = pl_loggers.TensorBoardLogger(f'./lightning_logs/{args.log_name}')  # log_name = multi_modal_t5

    # save checkpoint
    checkpoint_callback = ModelCheckpoint('./checkpoint/multi_modal_t5_attntype4_headnums4',
