...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None,
        ...
        elif cross_attn_type == 6: ### proposed method ###
            ## 이 부분은 linear_n에 대한 각각을 정의해주는거임!!
            ## forward 메서드는 아래에서!!
            self._linear_1 = nn.Linear(visual_feature_dim, dim_common) # K
            self._linear_2 = nn.Linear(visual_feature_dim, dim_common) # V
            self._linear_3 = nn.Linear(text_feature_dim, dim_common) # Q
            self._multi_head_attn_1 = nn.MultiheadAttention(dim_common, n_attn_heads)
            self._linear_4 = nn.Linear(text_feature_dim + dim_common, text_feature_dim) # 첫번째 멀티통과 output
            self._linear_5 = nn.Linear(audio_feature_dim, dim_common) # K
            self._linear_6 = nn.Linear(audio_feature_dim, dim_common) # V
            self._linear_7 = nn.Linear(text_feature_dim, dim_common) # Q
            self._multi_head_attn_2 = nn.MultiheadAttention(dim_common, n_attn_heads)
            self._linear_8 = nn.Linear(text_feature_dim + dim_common, text_feature_dim) # 두번째 멀티통과 output
            self._linear_9 = nn.Linear(dim_common, text_feature_dim)    # 만든부분
            if use_forget_gate:
                self.fg = nn.Linear(dim_common + text_feature_dim, dim_common)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if idx == self.fusion_layer:
                    ...
                    elif self.cross_attn_type == 6:     # 추가되는 부분         # batch 32                                               # batch 16
                        K1 = self._linear_1(image_features).transpose(0, 1)      # K (256, 32, 256)                                    (256, 16, 256)
                        V1 = self._linear_2(image_features).transpose(0, 1)      # V (256, 32, 256)                                    (256, 16, 256)
                        Q1 = self._linear_3(hidden_states).transpose(0, 1)       # Q (511, 64, 256)                                    (511, 32, 256)        # batch가 두배인 이유 : masked와 non-maksed 공존
                        attn_output, _ = self._multi_head_attn_1(Q1, K1, V1)        # attn_output (511, 64, 256)         
                        attn_output = attn_output.transpose(0, 1)               # (64, 511, 256)                                      (32, 511, 256)
                        if self.use_forget_gate:
                            forget_mask = self.fg(torch.cat((attn_output, hidden_states), 2))   # hs(64,511,768) attn_out(64,511,256)
                            forget_mask = self.sigmiod(forget_mask)             # forget_mask (64, 511, 256)
                            forget_mask = F.dropout(forget_mask, p=self.dropout, training=self.training)
                            attn_output = forget_mask.mul(attn_output)          # attn_output (64, 511, 256)                          (32, 511, 256)
                        output = self._linear_4(torch.cat((hidden_states, attn_output), 2))     # output (64, 511, 768) 드디어ㅠㅠㅠ!!ㅆㅂ완성  (32, 511, 768)
                        ## --- 여기까지가 첫번째 multi head attention --- ##
                                                                                # batch 32                                    batch 16
                        K2 = self._linear_5(audio_features).transpose(0, 1)      # K (256, 32, 43->256)                       (256, 16, 256)
                        V2 = self._linear_6(audio_features).transpose(0, 1)      # V (256, 32, 43->256)                       (256, 16, 256)
                        Q2 = self._linear_7(output).transpose(0, 1)       # Q (511, 64, 256)                                  (511, 32, 256)
                        attn_output, _ = self._multi_head_attn_2(Q2, K2, V2)         # attn_output (511, 64, 256)
                        attn_output = attn_output.transpose(0, 1)               # attn_output (64, 511, 256)
                        output = self._linear_8(torch.cat((hidden_states, attn_output), 2))     # output (64, 511, 768)      (32, 511, 768)
                        
                                         # output (64, 511, 256) -> (64, 511, 768)로!! 될까? 완료       

                        # cf. batch 32일때 hidden_states (64, 511, 768)   16일때 hidden_states (32, 511, 768)
                    ...
                    # Residual Connection
                    hidden_states = hidden_states + output                                 # (32, 511, 768)
                    hidden_states = self.final_layer_norm(hidden_states)
...
class BartForMultiModalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
            Labels for computing the masked  modeling loss. Indices should either be in ``[0, ...,
...
class BartForTriModalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"final_logits_bias",
        r"encoder\.version",
