...
def load_tf_weights_in_t5(model, config, tf_checkpoint_path):
    ...
    # tensorflow 체크포인트를 pytorch 모델에 로드하는 함수 
    try:
        import re
...
PARALLELIZE_DOCSTRING = r"""
    ...
    모델의 어텐션 모듈을 여러 장치에 분산시키기 위해 디바이스 맵을 사용합니다. 
    디바이스 맵이 지정되지 않은 경우, 모든 디바이스에 블록을 고르게 분산시킵니다.
    ...
    Args:
        device_map (:obj:`Dict[int, list]`, optional, defaults to None):
            ...
            following number of attention modules:
                ...
                - t5-large: 24
                - t5-3b: 24
                - t5-11b: 24        이게 모두 어탠션 모듈의 개수임

    Example::

            # Here is an example of a device map on a machine with 4 GPUs using t5-3b, which has a total of 24 attention modules:
            # 다음은 총 24개의 어탠션 모듈을 갖는 t5-3b 모델을 사용하는 4개의 gpu가 있는 기계에서 디바이스 맵을 사용하는 예이다.
...
DEPARALLELIZE_DOCSTRING = r"""
    ...
    Example::
        ...
        # deparallelize 함수를 사용하여 모델을 CPU로 다시 이동시키고, 
        # torch.cuda.empty_cache() 함수를 호출하여 생성된 GPU 캐시를 비울 수 있습니다.
"""


class T5LayerNorm(nn.Module):       # T5의 layerNorm을 구현한 클래스
    def __init__(self, hidden_size, eps=1e-6):  # hidden_size, epsilon 값을 입력으로 받음.
        """
    ...
    def forward(self, hidden_states):
        ...
        # 분산값에 epsilon을 더한값의 제곱근 값을 사용하고

        # convert into float16 if necessary
        if self.weight.dtype == torch.float16:
            hidden_states = hidden_states.to(torch.float16)
        return self.weight * hidden_states
        # 마지막으로 가중치 weight와 곱하여 출력

class T5DenseReluDense(nn.Module):  # 렐루
    ...
    def forward(self, hidden_states):
        hidden_states = self.wi(hidden_states)  # 선형변환인 self.wi 를 통해 d_model -> d_ff  차원맵핑
        hidden_states = F.relu(hidden_states)
        hidden_states = self.dropout(hidden_states) # 무작위로 뉴런을 끔
        hidden_states = self.wo(hidden_states)  # 다시 선형변환인 self.wo를 통해 d_ff -> d_model 매핑
        return hidden_states
        # 이렇게 T5DenseReluDense 클래스는 입력된 hidden_states에 대해 차원을 변환하고 
        # 비선형성을 추가하여 모델의 표현 능력을 향상시키고, 
        # dropout을 통해 모델을 일반화시키는 기능을 수행합니다.
...
class T5LayerFF(nn.Module):         # transformer의 feedforward 레이어
                                    # 기존 transformer와 다르게 T5는 Relu대신 Gated GELU 사용
                                    
    def __init__(self, config):
        super().__init__()
        if config.feed_forward_proj == "relu":
            self.DenseReluDense = T5DenseReluDense(config)
        elif config.feed_forward_proj == "gated-gelu":  # gated-gelu : Transformer 모델에서 자주 사용되는 함수
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False): 
        # false 라면 상대적인 어텐션 평향이 없다는 것을 의미함
        super().__init__()
        self.is_decoder = config.is_decoder
        self.has_relative_attention_bias = has_relative_attention_bias

        self.relative_attention_num_buckets = config.relative_attention_num_buckets
        # self.relative_attention_num_buckets는 상대적인 어텐션 편향에 사용되는 임베딩 테이블의 크기입니다.
        self.d_model = config.d_model   # 임베딩 벡터의 차원을 의미
        self.key_value_proj_dim = config.d_kv   # 각 헤드에서의 key,value 벡터의 차원
        self.n_heads = config.num_heads         # 어텐션 헤드의 수
        self.dropout = config.dropout_rate      # 드랍아웃 비율 
        self.inner_dim = self.n_heads * self.key_value_proj_dim     # 각 어텐션 헤드에서의 key, value 벡터의 크기

        # Mesh TensorFlow initialization to avoid scaling before softmax
        # 입력된 벡터를 각각 q,k,v 벡터로 선형변환 하는 레이어
        self.q = nn.Linear(self.d_model, self.inner_dim, bias=False)    
        self.k = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.v = nn.Linear(self.d_model, self.inner_dim, bias=False)
        self.o = nn.Linear(self.inner_dim, self.d_model, bias=False)
        # o 는 각 어텐션 헤드의 결과를 모아 최종 어텐션 결과를 계산하는 레이어

        if self.has_relative_attention_bias:
            self.relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets, self.n_heads)
        self.pruned_heads = set()
        # 어텐션 계산에서 제외된 어탠션 헤드의 집합

    def prune_heads(self, heads):   # prune = 삭제
        if len(heads) == 0:
            return
    ...
    @staticmethod # 첫번째 인자로 self를 받지 않으며, 클래스에서 직접 접근할 수 있다.
