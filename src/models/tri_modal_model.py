...
from models.modeling_bart import BartForTriModalGeneration
from datasets import load_metric

class BartTriModal(BaseModel):

    def __init__(self, args):
        self.args = args
        super(BartTriModal, self).__init__(args)
        self.model = BartForTriModalGeneration.from_pretrained('facebook/bart-base',
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, labels, image_features, image_len, audio_features, audio_len):
    ...
    def training_step(self, batch, batch_idx):
        # batch
        src_ids, decoder_ids, mask, label_ids, image_features, image_len, audio_features, audio_len = batch
        # get loss
        loss = self(input_ids=src_ids, attention_mask=mask, decoder_input_ids=decoder_ids, labels=label_ids, image_features=image_features.float(), image_len=image_len,
                    audio_features=audio_features.float(), audio_len=audio_len)
        # logs
