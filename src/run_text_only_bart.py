...
if __name__ == '__main__':
    ...
    parser.add_argument('-val_save_file', default='/root/VG-GPLMs/src/evaluation/temp_valid_file_text_only_bart', type=str, help='the validation results for each epoch') # 기존은 /temp_valid_file
    ...
    parser.add_argument('-batch_size', type=int, default=32, help='batch size for each gpu')     # 원래가 4 팀미팅에서 64
    ...
    => T_out = T + T_out (Residual Connection)
    '''
    parser.add_argument('-cross_attn_type', type=int, default=0)  # 현재 text_only_bart라서 0으로 넣어놈 # text_only_T5 0 하겠음. 
    parser.add_argument('-dim_common', type=int, default=256) ## 위에 attn_type을 개같이 놨었네;;; head로만 놨었음 ;;
    parser.add_argument('-n_attn_heads', type=int, default=1) # only needed when cross_attn_type is 4 or 5  # text_only_bart 1 하겠음. # 멀모bart 4로 하겠음
    ...
    # save checkpoint
    checkpoint_callback = ModelCheckpoint('./checkpoint/text_only_bart',
